-- First, ensure we have a home page
...
-- Update blocks for home page using a function
DO $$
DECLARE
  home_page_id uuid;
BEGIN
  ...
  DELETE FROM block WHERE page_id = home_page_id;

  -- Insert all blocks
  INSERT INTO block (page_id, key, title, content, sort) VALUES
    (home_page_id, 'hero', 
     '{"ar": "جمعية السكري اليمنية (YDA)", "en": "Yemen Diabetes Association (YDA)"}'::jsonb,
     '{"ar": "نُساندكم لتعيشوا بصحة أفضل مع السكري — نرفع الوعي، نُحسّن الرعاية، ونُعزّز قدرات المجتمع للوقاية والإدارة السليمة.", "en": "Helping people in Yemen live healthier with diabetes—awareness, better care, and strong community capacity."}'::jsonb, 1),
    
    (home_page_id, 'mission',
     '{"ar": "رسالتنا", "en": "Mission"}'::jsonb,
     '{"ar": "تمكين المصابين بالسكري وأُسرهم عبر التثقيف، والفحص المبكر، والإرشاد العلاجي، والدعم النفسي والاجتماعي.", "en": "Empower people living with diabetes and their families through education, early detection, clinical guidance, and psychosocial support."}'::jsonb, 2),
    
    (home_page_id, 'vision',
     '{"ar": "رؤيتنا", "en": "Vision"}'::jsonb,
     '{"ar": "يمن يتمتع فيه الجميع بفرص متكافئة للوقاية من السكري وإدارته، مع خفض المضاعفات وتحسين جودة الحياة.", "en": "A Yemen where everyone can prevent and manage diabetes, reducing complications and improving quality of life."}'::jsonb, 3),
    
    (home_page_id, 'values',
     '{"ar": "قيمنا", "en": "Values"}'::jsonb,
     '{"ar": "النزاهة · التميّز · الشراكة المجتمعية · العدالة في الوصول · الاستدامة", "en": "Integrity · Excellence · Community Partnership · Equity of Access · Sustainability"}'::jsonb, 4),
    
    (home_page_id, 'programs',
     '{"ar": "برامجنا وخدماتنا", "en": "Programs & Services"}'::jsonb,
     '{"ar": "التوعية المجتمعية: حملات ميدانية ورسائل رقمية ومواد تثقيفية مبسّطة.\n\nالفحص والإحالة المبكرة: أيام فحص HbA1c وسكر صائم مع تحويلات منظمة للرعاية.\n\nدعم المرضى والأُسر: تثقيف ذاتي، إرشاد غذائي وسلوكي ملائم للسياق اليمني، ودعم نفسي.\n\nتدريب العاملين الصحيين: تشخيص دقيق، رعاية القدم، متابعة منزلية، وبروتوكولات حديثة.\n\nالمدارس والشباب: مبادرات للوقاية المبكرة وتعزيز نمط الحياة الصحي.\n\nالبيانات والجودة: مؤشرات بسيطة لتحسين الجودة واستهداف البرامج.", "en": "Community awareness: field campaigns, digital messaging, and simple educational materials.\n\nEarly screening & referral: HbA1c and fasting glucose screening days with organized care transitions.\n\nPatient & family support: self-management education, nutrition and behavioral counseling adapted to Yemeni context, and psychosocial support.\n\nHealth-worker training: accurate diagnosis, foot care, home follow-up, and modern protocols.\n\nSchools & youth: initiatives for early prevention and promoting healthy lifestyles.\n\nData & quality: simple indicators to improve quality and target programs."}'::jsonb, 5),
    
    (home_page_id, 'about_diabetes',
     '{"ar": "عن السكري باختصار", "en": "About Diabetes in Brief"}'::jsonb,
     '{"ar": "السكري حالة مزمنة تنشأ عن نقص الإنسولين أو مقاومته. أعراض شائعة: عطش وتبوّل متكرران، إرهاق، بطء التئام الجروح، تشوش الرؤية، تنميل الأطراف.\n\nالوقاية ممكنة: غذاء متوازن، نشاط بدني منتظم، فحوصات دورية، والالتزام بالخطة العلاجية.", "en": "Diabetes is a chronic condition caused by insulin deficiency or resistance. Common symptoms: frequent thirst and urination, fatigue, slow wound healing, blurred vision, numbness in extremities.\n\nPrevention is possible: balanced diet, regular physical activity, periodic screenings, and adherence to treatment plan."}'::jsonb, 6),
    
    (home_page_id, 'how_diabetes_happens',
     '{"ar": "كيف يحدث السكري؟", "en": "How Does Diabetes Happen?"}'::jsonb,
     '{"ar": "السكري لا ينتقل بالعدوى؛ بل ينتج عن عوامل متداخلة تشمل الاستعداد الوراثي، العادات الغذائية غير الصحية، قلّة النشاط البدني، والسمنة/زيادة الوزن.", "en": "Diabetes is not contagious; it results from interrelated factors including genetic predisposition, unhealthy eating habits, lack of physical activity, and obesity/overweight."}'::jsonb, 7);
END $$;

-- Update KPIs - delete old and insert new
DELETE FROM kpi WHERE key IN ('adults_with_diabetes', 'adult_prevalence');

INSERT INTO kpi (key, value_int, year) VALUES
  ('adults_with_diabetes', 720800, 2024);

INSERT INTO kpi (key, value_dec, year) VALUES
  ('adult_prevalence', 5.5, 2024);

-- Delete existing conference event if exists and insert new
DELETE FROM event WHERE slug = '18th-annual-conference-2025';

INSERT INTO event (
  slug, title, summary, body, city, venue, start_at, end_at, status
) VALUES (
...
SET 
  phone = '+967 1 246 866',
  emails = '["info@yda.ngo"]'::jsonb,
  socials = '{"facebook": "https://www.facebook.com/61556779521467/", "youtube": "https://www.youtube.com/@YemenDiabetesAssociation"}'::jsonb,
  address = '{"ar": "صنعاء، اليمن", "en": "Sana''a, Yemen"}'::jsonb;
